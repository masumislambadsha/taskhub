...
export default function Sidebar({ role }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-primary z-40 flex flex-col transition-transform duration-300
          ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}
      >
        {/* Logo */}
        <div className="h-16 flex items-center px-6 border-b border-white/10 shrink-0">
          <Link href="/" onClick={() => dispatch(setSidebarOpen(false))}>
            <Logo size={32} light />
          </Link>
        </div>
        ...
        <div className="px-4 py-4 border-t border-white/10 shrink-0">
          <button
            ...
            className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium t hover:bg-red-500/10 hover:text-red-500 transition-all w-full text-red-300"
          >
